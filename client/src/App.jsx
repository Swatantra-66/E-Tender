  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="p-6">
        {!user && view === 'login' && (
          <div className="max-w-md mx-auto bg-white p-6 rounded shadow text-center">
            ...
            <div className="flex flex-col gap-3">
              <button onClick={()=>handleLogin('client')} className="px-4 py-2 bg-indigo-600 text-white rounded">Login as Client</button>
              <button onClick={()=>handleLogin('builder')} className="px-4 py-2 bg-green-600 text-white rounded">Login as Builder</button>
            </div>
          </div>
        )}
        {user && view === 'home' && (
        ...
        {user && view === 'post' && user.type==='client' && <PostProject user={user} />}
        {user && view === 'browse' && <BrowseProjects user={user} />}
        {user && view === 'builder' && user.type==='builder' && <BuilderDashboard user={user} />}
